...
export const generateCustomerReportHTML = (customer: any, tickets: any[], insight: any) => {
  ...
  const avgHandling = tickets.length > 0 
    ? (tickets.reduce((acc, t) => acc + (t.handlingDuration?.rawHours || 0), 0) / tickets.length).toFixed(1)
    : '0';
...
const generateTicketHistoryHTML = (tickets: any[]) => {
  ...
  const sortedKeys = Object.keys(groups).sort((a, b) => 
    Number(new Date(b + '-01')) - Number(new Date(a + '-01'))
  );
