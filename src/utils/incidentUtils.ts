...
const parseHHMMSS = (timeString: string): number => {
	...
	if (match) {
		...
		if (h >= 0 && h <= 23 && m >= 0 && m <= 59 && s >= 0 && s <= 59) {
			...
			logger.info(
				`Successfully parsed HH:MM:SS: "${timeString}" -> ${h}h ${m}m ${s}s -> ${totalMinutes} minutes`,
			);
			return totalMinutes;
		}
	}

	logger.warn(`Invalid HH:MM:SS format: "${timeString}"`);
	return 0;
};
...
export const toMinutes = (v: unknown): number => {
	...
	if (Number.isFinite(excelTime) && excelTime > 0 && excelTime < 1) {
		...
		logger.info(`Parsed Excel time serial: "${s}" -> ${totalMinutes} minutes`);
		return totalMinutes;
	}
	...
	if (hhmmMatch) {
		...
		if (h >= 0 && h <= 23 && m >= 0 && m <= 59) {
			...
			logger.info(
				`Parsed HH:MM format: "${s}" -> ${h}h ${m}m -> ${totalMinutes} minutes`,
			);
			return totalMinutes;
		}
	}
	...
	for (const format of timeFormats) {
		...
		if (match) {
			if (
				format.source.includes("h\\s*\\d+m") ||
				format.source.includes("\\d+h\\s*\\d+m")
			) {
				// Xh Ym format
				const [, hours, minutes] = match;
				const totalMinutes = +hours * 60 + +minutes;
				logger.info(
					`Parsed Xh Ym format: "${s}" -> ${hours}h ${minutes}m -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
				...
				logger.info(
					`Parsed Xh format: "${s}" -> ${hours}h -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
				...
				logger.info(
					`Parsed Xm format: "${s}" -> ${minutes}m -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
				...
				const totalMinutes = +hours * 60 + +minutes;
				logger.info(
					`Parsed "X hours Y minutes" format: "${s}" -> ${hours}h ${minutes}m -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
				...
				const totalMinutes = +hours * 60;
				logger.info(
					`Parsed "X hours" format: "${s}" -> ${hours}h -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
				...
				const totalMinutes = +minutes;
				logger.info(
					`Parsed "X minutes" format: "${s}" -> ${minutes}m -> ${totalMinutes} minutes`,
				);
				return totalMinutes;
			}
		}
	...
	if (Number.isFinite(n)) {
		if (n > 0 && n < 1000) {
			// Likely minutes
			logger.info(
				`Parsed numeric as minutes: "${s}" -> ${Math.round(n)} minutes`,
			);
			return Math.round(n);
		} else if (n >= 1000) {
			// Likely seconds, convert to minutes
			const minutes = Math.round(n / 60);
			logger.info(`Parsed numeric as seconds: "${s}" -> ${minutes} minutes`);
			return minutes;
		}
	}

	// If we reach here, the format wasn't recognized
	logger.warn(`Duration format not recognized: "${s}". Returning 0 minutes.`);
	return 0;
};
...
export const parseDateSafe = (dt?: string | Date | null): string | null => {
	...
	if (Number.isFinite(excelSerial) && excelSerial > 1000) {
		try {
			...
			logger.warn("Failed to convert Excel serial date:", excelSerial, error);
			return null;
		}
	}
	...
	for (let i = 0; i < formats.length; i++) {
		...
		const match = s.match(format);
		if (match) {
			logger.info(`✅ Matched format ${i + 1}: "${s}"`);
			try {
				if (match.length === 7) {
					...
					if (
						...
						logger.warn(`Invalid date/time components in: "${s}"`);
						continue;
					}
					...
					if (!isNaN(date.getTime())) {
						logger.info(
							`Successfully parsed date: "${s}" -> ${date.toISOString()}`,
						);
					...
					if (dayNum < 1 || dayNum > 31 || monthNum < 1 || monthNum > 12) {
						logger.warn(`Invalid date components in: "${s}"`);
						continue;
					}
				...
				logger.warn(`Error parsing date format: "${s}"`, error);
				continue;
			}
		}
	}
	...
	logger.info(`⚠️ No format matched, trying fallback for: "${s}"`);
	try {
		const date = new Date(s);
		if (!isNaN(date.getTime())) {
			logger.info(`✅ Fallback parsed date: "${s}" -> ${date.toISOString()}`);
			return date.toISOString();
		}
	} catch (error) {
		logger.warn(`❌ Fallback parsing failed for: "${s}"`, error);
	}

	logger.warn(`❌ Could not parse date: "${s}" - no valid format found`);
	return null;
};
...
export async function saveIncidentsChunked(rows: Incident[], chunkSize = 2000) {
	logger.info(
		`[saveIncidentsChunked] Starting to save ${rows.length} incidents in chunks of ${chunkSize}`,
	);
	...
	for (let i = 0; i < rows.length; i += chunkSize) {
		...
		logger.info(
			`[saveIncidentsChunked] Processing chunk ${chunkIndex}/${totalChunks} with ${part.length} incidents`,
		);
		...
		try {
			...
			if (ids.length !== uniqueIds.size) {
				logger.warn(
					`[saveIncidentsChunked] Duplicate IDs found in chunk ${chunkIndex}:`,
			...
			logger.info(
				`[saveIncidentsChunked] Chunk ${chunkIndex} saved successfully. Total saved so far: ${totalSaved}`,
			);

			// Verify the save by counting records
			const totalInDb = await db.incidents.count();
			logger.info(
				`[saveIncidentsChunked] Total incidents in database after chunk ${chunkIndex}: ${totalInDb}`,
			);
		} catch (error) {
			logger.error(
				`[saveIncidentsChunked] Error saving chunk ${chunkIndex}:`,
				error,
			);
			throw error;
		}
	...
	logger.info(
		`[saveIncidentsChunked] Final count in database: ${finalCount}, Expected: ${rows.length}`,
	);

	if (finalCount !== rows.length) {
		logger.warn(
			`[saveIncidentsChunked] DISCREPANCY DETECTED: Expected ${rows.length} but found ${finalCount} in database`,
		);
		...
		logger.info(
			`[saveIncidentsChunked] Unique incident IDs: ${uniqueIncidentIds.size}, Total incidents: ${allIncidents.length}`,
		);

		if (uniqueIncidentIds.size !== allIncidents.length) {
			logger.error(
				`[saveIncidentsChunked] DUPLICATE IDs FOUND: ${allIncidents.length - uniqueIncidentIds.size} duplicates`,
			);
		}
	}
...
export async function computeStats(range?: {
	...
	rows.forEach((r) => {
		const p = r.priority || "N/A";
		byPriority[p] = (byPriority[p] || 0) + 1;
		const k = r.klasifikasiGangguan || "N/A";
		byKlas[k] = (byKlas[k] || 0) + 1;
		const s = r.site || "N/A";
		bySite[s] = (bySite[s] || 0) + 1;
		const l = (r.level || "N/A").toString();
...
export async function queryIncidents(
	...
	if (filter.klasifikasiGangguan)
		rows = rows.filter(
			(r) => r.klasifikasiGangguan === filter.klasifikasiGangguan,
		);
...
export async function cleanDuplicateIncidents(): Promise<{
	removed: number;
	remaining: number;
}> {
	logger.info("[cleanDuplicateIncidents] Starting duplicate cleanup...");

	const allIncidents = await db.incidents.toArray();
	logger.info(
		`[cleanDuplicateIncidents] Found ${allIncidents.length} total incidents`,
	);
	...
	grouped.forEach((incidents, key) => {
		if (incidents.length > 1) {
			...
			logger.info(
				`[cleanDuplicateIncidents] Found ${incidents.length} duplicates for key "${key}", keeping first, removing ${toRemove.length}`,
			);
		}
	});
	...
	if (duplicatesToRemove.length > 0) {
		logger.info(
			`[cleanDuplicateIncidents] Removing ${duplicatesToRemove.length} duplicate incidents`,
		);
	...
	logger.info(
		`[cleanDuplicateIncidents] Cleanup complete. Removed: ${duplicatesToRemove.length}, Remaining: ${remaining}`,
	);
...
export async function getDatabaseStats(): Promise<{
	...
	const uniqueStartTimes = new Set(allIncidents.map((inc) => inc.startTime))
		.size;
...
export async function validateAndRepairDatabase(): Promise<{
	...
	logger.info("[validateAndRepairDatabase] Starting database validation...");
	...
	try {
		...
		for (const incident of allIncidents) {
			...
			if (!incident.id || !incident.noCase) {
				logger.warn(
					`[validateAndRepairDatabase] Invalid incident missing required fields:`,
					incident,
				);
			...
			if (incident.startTime) {
				try {
					...
					if (isNaN(startDate.getTime())) {
						logger.warn(
							`[validateAndRepairDatabase] Invalid startTime for incident ${incident.noCase}: ${incident.startTime}`,
						);
					...
					logger.warn(
						`[validateAndRepairDatabase] Error parsing startTime for incident ${incident.noCase}:`,
						error,
					);
					incident.startTime = null;
					needsUpdate = true;
			...
			if (incident.endTime) {
				try {
					...
					if (isNaN(endDate.getTime())) {
						logger.warn(
							`[validateAndRepairDatabase] Invalid endTime for incident ${incident.noCase}: ${incident.endTime}`,
						);
					...
					logger.warn(
						`[validateAndRepairDatabase] Error parsing endTime for incident ${incident.noCase}:`,
						error,
					);
					incident.endTime = null;
					needsUpdate = true;
			...
			if (
				typeof incident.durationMin !== "number" ||
				isNaN(incident.durationMin)
			) {
			...
			if (
				typeof incident.durationVendorMin !== "number" ||
				isNaN(incident.durationVendorMin)
			) {
			...
			if (
				typeof incident.totalDurationPauseMin !== "number" ||
				isNaN(incident.totalDurationPauseMin)
			) {
			...
			if (
				typeof incident.totalDurationVendorMin !== "number" ||
				isNaN(incident.totalDurationVendorMin)
			) {
			...
			if (
				typeof incident.netDurationMin !== "number" ||
				isNaN(incident.netDurationMin)
			) {
			...
			if (
				typeof incident.powerBefore !== "number" ||
				isNaN(incident.powerBefore)
			) {
			...
			if (
				typeof incident.powerAfter !== "number" ||
				isNaN(incident.powerAfter)
			) {
		...
		if (incidentsToDelete.length > 0) {
			logger.info(
				`[validateAndRepairDatabase] Deleting ${incidentsToDelete.length} invalid incidents`,
			);
		...
		if (incidentsToUpdate.length > 0) {
			logger.info(
				`[validateAndRepairDatabase] Updating ${incidentsToUpdate.length} repaired incidents`,
			);
			await db.incidents.bulkPut(incidentsToUpdate);
		}

		logger.info(
			`[validateAndRepairDatabase] Validation complete. Valid: ${validIncidents}, Invalid: ${invalidIncidents}, Repaired: ${repairedIncidents}`,
		);
		...
		logger.error("[validateAndRepairDatabase] Error during validation:", error);
...
export const calculateIncidentStats = (incidents: any[]) => {
	...
	const incidentsWithNetDuration = incidents.filter(
		(i) => calculateNetDuration(i) > 0,
	);
	...
	const ncalCounts = incidents.reduce(
		(acc, incident) => {
	...
	const siteCounts = incidents.reduce(
		(acc, incident) => {
...
export const filterIncidents = (incidents: any[], filter: any) => {
	...
	if (filter.priority) {
		filtered = filtered.filter(
			(incident) => incident.priority === filter.priority,
		);
	}
	...
	if (filter.ncal) {
		filtered = filtered.filter(
			(incident) => normalizeNCAL(incident.ncal) === filter.ncal,
		);
	}
	...
	if (filter.klasifikasiGangguan) {
		filtered = filtered.filter(
			(incident) => incident.klasifikasiGangguan === filter.klasifikasiGangguan,
		);
	}
