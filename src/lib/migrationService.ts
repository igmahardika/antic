...
export class MigrationService {
	...
	async migrateTickets(): Promise<void> {
		try {
			...
			this.reportProgress(
				...
				0,
				tickets.length,
				`Found ${tickets.length} tickets to migrate`,
			);
			...
			logger.error("Ticket migration error:", error);
			throw new Error(
				`Failed to migrate tickets: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async migrateCustomers(): Promise<void> {
		try {
			this.reportProgress(
				...
				0,
				0,
				"Loading customers from IndexedDB...",
			);
			...
			if (customers.length === 0) {
				this.reportProgress(
					...
					0,
					0,
					"No customers found in IndexedDB",
				);
				return;
			}
			...
			this.reportProgress(
				...
				0,
				customers.length,
				`Found ${customers.length} customers to migrate`,
			);
			...
			logger.error("Customer migration error:", error);
			throw new Error(
				`Failed to migrate customers: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async migrateAll(): Promise<void> {
		try {
			logger.info("🚀 Starting full migration from IndexedDB to MySQL...");
