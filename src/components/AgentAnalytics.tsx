...
const AgentAnalytics = () => {
	...
	const agentMetrics = useAgentStore(
		(state) => state.agentMetrics,
	) as AgentMetric[];
	...
	const [debouncedDatasets, setDebouncedDatasets] = useState<
	...
	useEffect(() => {
		if (dataSource && dataSource.length > 0) {
			logger.info(
				"Agent names in dataSource:",
				dataSource.map((a) => a.agent),
			);
		}
	}, [dataSource]);
	...
	const exportToPDF = async () => {
		...
		try {
			...
			addHeader(
				"Agent Career Report",
				`Performance Analysis for ${selectedAgent}`,
			);
			...
			pdf.setTextColor(107, 114, 128);
			pdf.text(
				`Report Period: ${exportYear === "all" ? "All Time Career Data" : exportYear}`,
				margin,
				yPosition,
			);
			pdf.text(
			...
			// Executive Summary
			addParagraph(
				`This report provides a comprehensive analysis of ${selectedAgent}'s performance throughout their career. The analysis covers key performance indicators, efficiency metrics, quality standards, and actionable insights for continuous improvement.`,
			);
			...
			const avgAHT =
				ahtValues.length > 0
					? ahtValues.reduce((sum, val) => sum + val, 0) / ahtValues.length
					: 0;
			...
			const fcrRate =
				totalTickets > 0 ? (fcrTickets.length / totalTickets) * 100 : 0;
			...
			const slaCompliant = filteredTickets.filter((t) => {
				...
				if (isNaN(openDate.getTime()) || isNaN(handlingDate.getTime()))
					return false;
			...
			const slaRate =
				totalTickets > 0 ? (slaCompliant / totalTickets) * 100 : 0;
			...
			const escalationRate =
				totalTickets > 0 ? (escalated / totalTickets) * 100 : 0;
			...
			const shiftData = [
				{
					...
					percentage: ((shiftCount.Pagi / totalTickets) * 100).toFixed(1),
				},
				{
					shift: "Siang",
					count: shiftCount.Siang,
					percentage: ((shiftCount.Siang / totalTickets) * 100).toFixed(1),
				},
				{
					shift: "Malam",
					count: shiftCount.Malam,
					percentage: ((shiftCount.Malam / totalTickets) * 100).toFixed(1),
				},
			];
			...
			pdf.setTextColor(107, 114, 128);
			pdf.text(
				"Generated by Helpdesk Management System",
				pageWidth / 2,
				yPosition,
				{ align: "center" },
			);
	...
	// }, [data]);
	const agentTrendDatasets: {
		label: string;
		data: number[];
		color?: string;
	}[] = data?.agentMonthlyChart?.datasets || [];
	...
	useEffect(() => {
		const timer = setTimeout(() => {
			...
			setDebouncedTrendData(
				toRechartsAgentTrend(windowedLabels, windowedDatasets),
			);
		...
		return (
			<div className="flex flex-col items-center justify-center h-full text-gray-500 p-12">
				...
				<p className="text-base font-medium">
					Please upload a file to see agent analytics.
				</p>
			</div>
		);
	}
	...
	function calculateAgentScore(agent: any, maxTicket: number) {
		...
		const ticketScore = scoreTicket(agent.ticket, maxTicket) * 0.1;
		return (
	...
	function getAgentScoreTrend(agentName: string) {
		...
		const idx = data.agentMonthlyChart.datasets.findIndex(
			(ds) => ds.label === agentName,
		);
		if (idx === -1) return [];
		// Ambil data bulanan agent
		const volArr = data.agentMonthlyChart.datasets[idx]?.data || [];
		const frtArr =
			data.agentMonthlyChart.datasetsFRT?.find((ds) => ds.label === agentName)
				?.data || [];
		const artArr =
			data.agentMonthlyChart.datasetsART?.find((ds) => ds.label === agentName)
				?.data || [];
		const fcrArr =
			data.agentMonthlyChart.datasetsFCR?.find((ds) => ds.label === agentName)
				?.data || [];
		const slaArr =
			data.agentMonthlyChart.datasetsSLA?.find((ds) => ds.label === agentName)
				?.data || [];
		const backlogArr =
			data.agentMonthlyChart.datasetsBacklog?.find(
				(ds) => ds.label === agentName,
			)?.data || [];
	...
	const topOverall = agentWithScore.reduce(
		(a, b) => (b.score > a.score ? b : a),
		agentWithScore[0],
	);
	const fastestResponder = agentWithScore.reduce(
		(a, b) => (b.frt < a.frt ? b : a),
		agentWithScore[0],
	);
	const fastestResolution = agentWithScore.reduce(
		(a, b) => (b.art < a.art ? b : a),
		agentWithScore[0],
	);
	const bestSLA = agentWithScore.reduce(
		(a, b) => (b.sla > a.sla ? b : a),
		agentWithScore[0],
	);
	const mostReliable = agentWithScore
		.filter((a) => a.backlog === 0)
		.reduce((a, b) => (b.fcr > a.fcr ? b : a), agentWithScore[0]);
	const mostEngaged = agentWithScore.reduce(
		(a, b) => (b.vol > a.vol ? b : a),
		agentWithScore[0],
	);
	...
	const mostImproved = agentWithDelta.reduce(
		...
		<PageWrapper maxW="4xl">
				...
				<div className="flex justify-between items-center mb-8">
					<div>
						...
						<p className="text-base text-muted-foreground">
							Comprehensive performance analysis and insights for all agents
						</p>
					</div>
			...
			<div className="mb-8">
				...
				<p className="text-sm text-muted-foreground">
					Click on any agent card to view detailed performance analysis
				</p>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
				{sortedAgentWithScore.map((agent) => {
					...
					return (
						<div
							key={agent.agent}
							className="relative bg-white dark:bg-zinc-900 text-card-foreground rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden min-h-[520px] flex flex-col agent-card border border-zinc-200 dark:border-zinc-800"
							onClick={() => {
									...
									<div className="w-full h-full flex items-center justify-center">
										<img
											...
											onError={(e) => {
												...
												logger.warn(
													`[Photo Error] Failed to load image for agent: "${agent.agent}" at path: ${target.src}`,
												);
											...
											onLoad={() => {
												logger.info(
													`[Photo Success] Successfully loaded image for agent: "${agent.agent}"`,
												);
											}}
										/>
								...
								<div className="w-2/3 flex-1 p-6 flex flex-col justify-between min-w-0 content-section">
									...
									<div className="space-y-4">
										...
										<div className="border-b border-zinc-100 dark:border-zinc-800 pb-3">
											<div className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-1 line-clamp-2 agent-name leading-tight">
												{agent.agent}
											</div>
										...
										<div className="grid grid-cols-2 gap-3">
											...
											<div
												className={`rounded-lg p-3 text-center shadow-sm ${scoreBox.bg} bg-gradient-to-br ${scoreBox.bg === "bg-green-500" ? "from-green-500 to-emerald-600" : scoreBox.bg === "bg-blue-500" ? "from-blue-500 to-indigo-600" : scoreBox.bg === "bg-orange-500" ? "from-orange-500 to-amber-600" : "from-red-500 to-rose-600"}`}
											>
												<div
													className={`text-xl font-bold ${scoreBox.valueColor} mb-1`}
												>
													{agent.score ?? 0}
												</div>
												<div
													className={`text-xs font-medium ${scoreBox.valueColor === "text-white" ? "text-white/90" : "text-zinc-600"}`}
												>
													Performance Score
												</div>
											</div>
										...
										<div className="grid grid-cols-3 gap-3">
											...
											<div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-3 text-center metric-card hover:bg-zinc-100 dark:hover:bg-zinc-800/70">
												...
												<div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 mb-1">
													{agent.frt ? formatDurationDHM(agent.frt) : "-"}
												</div>
												<div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
													FRT
												</div>
											...
											<div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-3 text-center metric-card hover:bg-zinc-100 dark:hover:bg-zinc-800/70">
												<div className="flex justify-center mb-2">
													<AccessTimeIcon
														className="text-red-600 dark:text-red-400 mb-1"
														fontSize="small"
													/>
												</div>
												<div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 mb-1">
											...
											<div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-3 text-center metric-card hover:bg-zinc-100 dark:hover:bg-zinc-800/70">
												<div className="flex justify-center mb-2">
													<MoveToInboxIcon
														className="text-rose-600 dark:text-rose-400 mb-1"
														fontSize="small"
													/>
												</div>
												<div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 mb-1">
			...
			<RadixDialog.Root
				...
				onOpenChange={(open) => {
					setModalOpen(open);
					if (!open) setSelectedAgent(open ? selectedAgent : null);
				}}
			>
				<RadixDialog.Portal>
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-6">
									...
									<div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 flex items-center justify-center relative overflow-hidden shadow-sm">
										<img
											src={
												selectedAgent ? getAgentPhotoPath(selectedAgent) : ""
											}
											alt={selectedAgent}
											...
											onError={(e) => {
												const target = e.target as HTMLImageElement;
												logger.warn(
													`[Modal Photo Error] Failed to load image for selected agent: "${selectedAgent}" at path: ${target.src}`,
												);
												target.style.display = "none";
												target.nextElementSibling?.classList.remove("hidden");
											}}
											onLoad={() => {
												logger.info(
													`[Modal Photo Success] Successfully loaded image for selected agent: "${selectedAgent}"`,
												);
											}}
										/>
								...
								<div className="flex items-center gap-3">
									...
									<RadixDialog.Close asChild>
										<button
											className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 text-xl font-light focus:outline-none transition-colors duration-200 p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800"
							...
							<>
								...
								<div className="px-6 pb-6">
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
										{(() => {
											const agentData = sortedAgentWithScore.find(
												...
												<>
													<div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-4">
														<div className="flex items-center gap-3">
															...
															<div>
																<div className="text-xs text-zinc-600 dark:text-zinc-400 font-medium mb-1">
																	Rank
																</div>
													...
													<div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-4">
														<div className="flex items-center gap-3">
															<div
																className={`w-10 h-10 rounded-lg ${agentData.score >= 75 ? "bg-green-500" : agentData.score >= 60 ? "bg-blue-500" : agentData.score >= 45 ? "bg-orange-500" : "bg-red-500"} flex items-center justify-center`}
															>
																<span className="text-white text-sm font-semibold">
																	{agentData.score}
																</span>
								...
								{(() => {
									...
									if (scoreTrendArr.length > 1) {
										const diff =
											scoreTrendArr[scoreTrendArr.length - 1] -
											scoreTrendArr[scoreTrendArr.length - 2];
										if (diff > 0)
											trendBadge = (
												<Badge variant="success" className="ml-2 rounded-lg">
													Trend Up
												</Badge>
											);
										else if (diff < 0)
											trendBadge = (
												<Badge variant="danger" className="ml-2 rounded-lg">
													Trend Down
												</Badge>
											);
										else
											trendBadge = (
												<Badge variant="default" className="ml-2 rounded-lg">
													Stable
												</Badge>
											);
									}
									...
									if (
										scoreTrendArr.length &&
										agentMonthlyChart &&
										agentMonthlyChart.labels
									) {
										scoreTrendArr.forEach((v, i) => {
											if (v > bestValue) {
												bestValue = v;
												bestMonth = agentMonthlyChart.labels[i];
											}
											if (v < worstValue) {
												worstValue = v;
												worstMonth = agentMonthlyChart.labels[i];
											}
										});
									}
									// Overall trend
									let mainTrend = "";
									if (scoreTrendArr.length > 1) {
										const first = scoreTrendArr[0],
											last = scoreTrendArr[scoreTrendArr.length - 1];
										if (last > first)
											mainTrend = "Performa meningkat secara keseluruhan.";
										else if (last < first)
											mainTrend = "Performa menurun secara keseluruhan.";
									...
									const ticketArr =
										agentMonthlyChart?.datasets?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									const frtArr =
										agentMonthlyChart?.datasetsFRT?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									const artArr =
										agentMonthlyChart?.datasetsART?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									const fcrArr =
										agentMonthlyChart?.datasetsFCR?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									const slaArr =
										agentMonthlyChart?.datasetsSLA?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									const backlogArr =
										agentMonthlyChart?.datasetsBacklog?.find(
											(ds) => ds.label === selectedAgent,
										)?.data || [];
									// --- Tambahan: total tiket per bulan & persentase handle agent ---
									const totalTicketPerMonth =
										agentMonthlyChart?.totalTicketsPerMonth || [];
												...
												<div>
													...
													<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
														{Object.keys(kpiTrends).map((kpi) => (
															<div
																key={kpi}
																className="flex items-center gap-3 p-3 bg-white dark:bg-zinc-800 rounded-lg"
															>
																<Badge
																	variant={
																		kpiTrends[kpi] === "up"
																...
																<div className="flex-1">
																	...
																	<div className="text-sm text-zinc-600 dark:text-zinc-400 font-medium">
																		{generateKpiInsight(kpi, kpiTrends[kpi])}
																	</div>
																	{kpi === "ticket" &&
																		agentTicketShare.length > 0 && (
																			<div className="text-xs text-zinc-500 mt-1 font-medium">
																				Share: {agentTicketShare.slice(-1)[0]}{" "}
																				of total tickets
																			</div>
																		)}
																</div>
									...
									<Tabs defaultValue="overview" className="w-full">
										...
										<TabsContent value="overview" className="mt-6">
											<div className="mb-6">
												...
												<p className="text-sm text-zinc-600 dark:text-zinc-400">
													Comprehensive summary of agent's career performance
													and metrics
												</p>
											</div>
											{(() => {
												...
												if (agentTickets.length === 0) {
													return (
														<div className="text-center py-8 text-gray-500">
															...
															<p className="text-base font-medium">
																No career data available for this agent.
															</p>
														</div>
													);
												}
															...
															<CardContent className="p-6">
																<div className="flex items-center justify-between">
																	<div>
																		...
																		<p className="text-sm text-zinc-600 dark:text-zinc-400">
																			Comprehensive career summary and
																			performance index
																		</p>
																	</div>
											...
											{(() => {
												...
												const frtValues = agentTickets
													...
													.map((t) => {
														...
														return (
															(handling.getTime() - open.getTime()) / 60000
														);
													})
												...
												if (typeof window !== "undefined" && selectedAgent) {
													logger.info(
														`[Agent Career Report] ${selectedAgent} - Data Validation:`,
														{
															totalTickets,
																	...
																	<div className="flex items-center gap-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
																		...
																		<div>
																			...
																			<p className="text-sm text-muted-foreground">
																				{totalTickets >= 1000 ? (
																					<>
																						<EmojiEventsIcon className="w-4 h-4 inline mr-1" />{" "}
																						1000+ Tickets Handled
																					</>
																				) : totalTickets >= 500 ? (
																					<>
											...
											{(() => {
												...
												if (slaRate < 85) {
													insights.push({
														...
														recommendation:
															"Focus on faster ticket resolution and prioritize urgent cases.",
														color:
															"bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800",
													});
												} else if (slaRate > 95) {
												...
												if (fcrRate < 75) {
													insights.push({
														...
														recommendation:
															"Review knowledge base and improve first-contact resolution skills.",
														color:
															"bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800",
													});
												}
												...
												if (totalTickets > 500) {
													insights.push({
														...
														recommendation:
															"Consider mentoring opportunities for newer agents.",
														color:
													...
													<div className="space-y-6">
														<Card>
															<CardHeader>
																...
																<p className="text-sm text-muted-foreground">
																	AI-powered insights based on performance
																	analysis
																</p>
															</CardHeader>
															<CardContent>
															...
															<table className="min-w-full text-sm">
																...
																<tbody className="space-y-2">
																	{(() => {
																		...
																		// Calculate SLA compliance with validation - using closeHandling
																		const slaCompliant = agentTickets.filter(
																			(t) => {
																		...
																		const fcrTickets = agentTickets.filter(
																			(t) => {
																				return (
																					!t.handling2 ||
																					t.handling2.trim() === ""
																		...
																		const escalated = agentTickets.filter(
																			(t) => {
																				const handlingFields = [
																					t.closeHandling2,
																					t.closeHandling3,
																		...
																		const metrics = [
																			...
																			{
																				name: "SLA Compliance",
																				value: `${((slaCompliant / totalTickets) * 100).toFixed(1)}%`,
																				target: "85%",
																				status:
																					(slaCompliant / totalTickets) * 100 >=
																					85
																			...
																			{
																				name: "FCR Rate",
																				value: `${((fcrTickets.length / totalTickets) * 100).toFixed(1)}%`,
																				target: "75%",
																				status:
																			...
																			},
																			{
																				...
																				value: `${((escalated / totalTickets) * 100).toFixed(1)}%`,
																				target: "<10%",
																				status:
																					(escalated / totalTickets) * 100 < 10
																						? "success"
																						: "warning",
																			},
																		];
