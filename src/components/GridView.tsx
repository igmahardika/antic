...
const columns = [
  ...
  { key: 'openTime', label: 'Open Time', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'closeTime', label: 'Close Time', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'duration', label: 'Duration', width: '120px', render: (_v: any, row: any) => renderDuration(row.duration) },
  { key: 'closeHandling', label: 'Close Penanganan', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration', label: 'Durasi Penanganan (ART)', width: '140px', render: (_v: any, row: any) => renderDuration(row.handlingDuration) },
  { key: 'handling1', label: 'Penanganan 1', width: '180px' },
  { key: 'closeHandling1', label: 'Close Penanganan 1', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration1', label: 'Durasi Penanganan 1 (FRT)', width: '140px', render: (_v: any, row: any) => renderDuration(row.handlingDuration1) },
  { key: 'handling2', label: 'Penanganan 2', width: '180px' },
  { key: 'closeHandling2', label: 'Close Penanganan 2', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration2', label: 'Durasi Penanganan 2', width: '140px', render: (_v: any, row: any) => <span className="text-xs">{row.handlingDuration2?.formatted || ''}</span> },
  { key: 'handling3', label: 'Penanganan 3', width: '180px' },
  { key: 'closeHandling3', label: 'Close Penanganan 3', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration3', label: 'Durasi Penanganan 3', width: '140px', render: (_v: any, row: any) => <span className="text-xs">{row.handlingDuration3?.formatted || ''}</span> },
  { key: 'handling4', label: 'Penanganan 4', width: '180px' },
  { key: 'closeHandling4', label: 'Close Penanganan 4', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration4', label: 'Durasi Penanganan 4', width: '140px', render: (_v: any, row: any) => <span className="text-xs">{row.handlingDuration4?.formatted || ''}</span> },
  { key: 'handling5', label: 'Penanganan 5', width: '180px' },
  { key: 'closeHandling5', label: 'Close Penanganan 5', width: '140px', render: (v: any) => v ? <span className="text-xs">{formatDateTimeDDMMYYYY(v)}</span> : '' },
  { key: 'handlingDuration5', label: 'Durasi Penanganan 5', width: '140px', render: (_v: any, row: any) => <span className="text-xs">{row.handlingDuration5?.formatted || ''}</span> },
...
const GridView = ({ data: propsData }: { data?: ITicket[] }) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (result.length > 0 && (monthFilter !== 'all' || yearFilter !== 'all')) {
      console.log('Sample openTime formats:', result.slice(0, 3).map(r => r.openTime));
    }
    if (search) {
    ...
    if (monthFilter !== 'all' || yearFilter !== 'all') {
      result = result.filter(row => {
        ...
        if (result.indexOf(row) < 3) {
          console.log(`Filter debug - openTime: "${row.openTime}", month: "${month}", year: "${year}", monthFilter: "${monthFilter}", yearFilter: "${yearFilter}", monthMatch: ${monthMatch}, yearMatch: ${yearMatch}`);
        }
    ...
    return result;
  }, [data, search, validasiFilter, customerNames, durationFilter, monthFilter, yearFilter]);
            ...
            <div>
              ...
              <CardHeaderDescription className="text-xs mt-1">
                Showing {paged.length} of {filtered.length} tickets
                                 {validasiFilter !== 'all' && (
                   <Badge variant="info" className="ml-2 text-[8px] px-1 py-0.5 opacity-90">
                     {validasiFilter === 'valid' ? 'Valid Customers Only' : 'Invalid Customers Only'}
                   </Badge>
                 )}
                 {durationFilter !== 'all' && (
                   <Badge variant="warning" className="ml-2 text-[8px] px-1 py-0.5 opacity-90">
                 ...
                 {monthFilter !== 'all' && (
                   <Badge variant="secondary" className="ml-2 text-[8px] px-1 py-0.5 opacity-90">
                            ...
                            {columns.map(col => (
                              ...
                              >
                                ...
                                {is2025 && !isValid && col.key === 'name' && (
                                  <Badge variant="danger" className="mt-1 text-[8px] px-1 py-0.5 opacity-90">
                                    Invalid Customer
                                  </Badge>
                                )}
                              </td>
