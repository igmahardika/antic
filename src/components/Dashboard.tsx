...
const Dashboard = () => {
	...
	const { ticketAnalyticsData, gridData } = useMemo(() => {
		...
		const agentPerformance: {
			[key: string]: { durations: number[]; closed: number };
		} = {};
		...
		const agentMonthlyPerformance: {
			[agentName: string]: { [month: string]: number };
		} = {};
		...
		const totalDuration = gridData.reduce(
		...
		const monthlyStats: {
			[key: string]: { incoming: number; closed: number };
		} = {};
		gridData.forEach((ticket) => {
			try {
		...
		const topComplaintsTableData = Object.entries(categoryDetails)
			.map(([category, data]) => {
				const totalDuration = data.tickets.reduce(
					(acc, t) => acc + (t.duration?.rawHours || 0),
					0,
				);
				const avgDuration =
		...
		gridData.forEach((t) => {
			...
			customerTicketCounts[customer] =
				(customerTicketCounts[customer] || 0) + 1;
		});
		...
		const ticketAnalyticsData = {
			stats: [
				...
				},
				{
					...
					value: formatDurationDHM(totalDuration / totalTickets) || "00:00:00",
				...
				{
					title: "Active Agents",
					value: finalAgentData.summary.totalAgents.toString(),
	...
	useEffect(() => {
		...
		const isAgentAnalyticsPage =
			currentPath.includes("/agent-analytics") ||
			currentPath.includes("/ticket/agent-analytics");
	...
	const tabs = useMemo(() => {
		try {
			...
			logger.error("Failed to parse permissions from localStorage", e);
		}
	...
	return (
		<PageWrapper maxW="4xl">
			...
			<SummaryDashboard
				ticketAnalyticsData={ticketAnalyticsData}
				filteredTickets={gridData}
				standalone={false}
			/>
		</PageWrapper>
	);
