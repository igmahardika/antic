					...
					<Select.Viewport>
						{options.map((opt) => (
							<Select.Item
								key={opt.value}
								value={opt.value}
								className="px-3 py-2 cursor-pointer hover:bg-accent rounded flex items-center text-foreground"
							>
		...
		<div className="flex items-center gap-3 p-2 bg-background/70 rounded-xl shadow  mb-4">
			...
			<RadixSelect
				...
				options={[
					...
					...allYearsInData.map((y) => ({ value: y, label: y })),
				]}
				placeholder="Year"
			/>
		</div>
