...
export default function AppSidebar() {
                ...
                {menu.children.map((child: any) => (
                  <SidebarMenuSubItem key={child.path}>
                    <SidebarMenuSubButton asChild>
                      <NavLink to={child.path} className={({ isActive }) => (isActive ? 'text-sidebar-primary' : undefined)}>
                        {child.icon}
    ...
    return (
      <SidebarMenuItem key={menu.path}>
        <SidebarMenuButton asChild>
          <NavLink to={menu.path} className={({ isActive }) => (isActive ? 'text-sidebar-primary' : undefined)}>
            {menu.icon}
            <span>{menu.name}</span>
          </NavLink>
        </SidebarMenuButton>
  ...
  const otherMenus = allMenus.filter((m) => ['Documentation', 'Admin Panel'].includes(m.name))
