...
>(({ className, style, ...props }, ref) => {
	...
	const forcedStyle = isActive
		? {
				background: "#5271ff",
				color: "#fff",
				boxShadow: "0 8px 32px 0 rgba(82,113,255,0.25)",
...
export function SidebarNav({
	...
	const handleMouseLeave = () => {
		...
		window.dispatchEvent(
			new CustomEvent("sidebar:collapse", { detail: { collapsed: true } }),
		);
		...
		if (menu.children && menu.children.length > 0) {
			return (
				<div key={menu.path} className={parent ? "" : "mb-2"}>
					...
					<div className={`${autoCollapsed ? "hidden" : "block"} ml-4`}>
						{" "}
						{/* Indent submenu */}
