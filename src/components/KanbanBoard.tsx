...
import { LineChart, Line, ResponsiveContainer, XAxis, YAxis, LabelList, PieChart, Pie, Cell, Legend } from 'recharts';
...
const KanbanBoard = (props: Partial<KanbanBoardProps>) => {
  ...
  const { allTickets, cutoffStart, cutoffEnd, startMonth, setStartMonth, endMonth, setEndMonth, selectedYear, setSelectedYear } = analytics;
  ...
  const CustomerReportPDF = ({ customer, insight, historical, tickets }) => {
        ...
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.table}>
            ...
            {sortedTicketKeys.length === 0 ? (
              ...
              sortedTicketKeys.map((key, groupIdx) => {
                const [yyyy, mm] = key.split('-');
          ...
          {/* Footer */}
          <Text style={styles.footer}>Generated by ANTIC Analytics Dashboard</Text>
        </Page>
      </Document>
    );
  };
            ...
            {selectedCustomer && (
              <>
                ...
                <div className="px-10 pb-8 flex justify-end">
                  <PDFDownloadLink document={<CustomerReportPDF customer={selectedCustomer} insight={generateInsight(selectedCustomer.allTickets)} historical={[]} tickets={selectedCustomer.allTickets} />} fileName={`CustomerReport-${selectedCustomer.name}.pdf`}>
                    {({ loading }) => (
