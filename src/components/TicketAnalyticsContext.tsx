...
export const TicketAnalyticsProvider = ({ children }) => {
	...
	if (typeof window !== "undefined") {
		logger.info("[DEBUG] Open tickets analysis:", {
	...
	const overdueTickets = gridData.filter(
		(t) => Number(t.duration?.rawHours) > 24,
	).length;
	...
	const classificationAnalysis: Record<
		string,
		{
			count: number;
			sub: Record<string, number>;
			trendlineRaw?: Record<string, number>;
	...
	if (typeof window !== "undefined" && gridData.length > 0) {
		...
		logger.info("[DEBUG] Manual open test on samples:", manualOpenTest);
	}
	...
	gridData.forEach((ticket) => {
		try {
			...
			logger.error("[DEBUG] Error processing ticket:", e);
		}
	});

	// Debug: Log monthlyStats
	if (typeof window !== "undefined") {
		logger.info("[DEBUG] Monthly Stats:", monthlyStats);
	}
	...
	const categoryDetails: Record<
		string,
		{ tickets: typeof gridData; subCategories: Record<string, number> }
	> = {};
	gridData.forEach((t) => {
		const category = t.category || "Lainnya";
	...
	const ticketAnalyticsData = {
		stats: [
			...
			{
				title: "Open",
				value: finalOpenTickets.toString(),
				description:
					"Tiket yang masih terbuka (status bukan closed dan closeTime kosong atau di bulan berikutnya)",
			},
			{
				title: "Overdue",
	...
	useEffect(() => {
		if (gridData.length === 0) {
			...
			logger.info("[TicketAnalyticsContext] Loaded tickets:", {
