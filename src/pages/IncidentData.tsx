...
export const IncidentData: React.FC = () => {
	...
	const normalizeNCAL = (ncal: string | null | undefined): string => {
		...
		switch (normalized) {
			...
			case "black":
				return "Black";
			default:
				return ncal.trim(); // Return original if not recognized
		}
	};
	...
	React.useEffect(() => {
		logger.info("AllIncidents updated:", allIncidents?.length || 0);
		if (allIncidents && allIncidents.length > 0) {
			logger.info("Sample incident:", allIncidents[0]);
			logger.info("NCAL values in data:", [
	...
	const uniquePriorities = [
		...new Set(allIncidents?.map((i) => i.priority).filter(Boolean) || []),
	];
	...
	useEffect(() => {
		const loadData = async () => {
			...
			try {
				...
				setDbStats(stats);
				logger.info("[IncidentData] Database stats:", stats);
			} catch (error) {
	...
	const handleRecalculateDurations = async () => {
		...
		try {
			logger.info(
				"🔄 Starting duration recalculation for",
				allIncidents.length,
				"incidents...",
			);
			...
			const calculateDuration = (
				...
				try {
					...
					logger.warn("Error calculating duration:", error);
					return 0;
				}
			};
			...
			allIncidents.forEach((incident) => {
				...
				if (incident.startTime && incident.endTime) {
					...
					if (calculatedDuration > 0) {
						updatedIncident.durationMin =
							Math.round(calculatedDuration * 100) / 100;
						needsUpdate = true;
						recalculatedCount++;
					} else {
				...
				if (incident.startPause1 && incident.endPause1) {
					const pause1Duration = calculateDuration(
				...
				if (incident.startPause2 && incident.endPause2) {
					const pause2Duration = calculateDuration(
				...
				updatedIncident.totalDurationPauseMin =
					Math.round(totalPauseMinutes * 100) / 100;
				needsUpdate = true;
				...
				updatedIncident.totalDurationVendorMin =
					Math.round(totalVendorDuration * 100) / 100;
				needsUpdate = true;
			...
			if (incidentsToUpdate.length > 0) {
				logger.info(`💾 Updating ${incidentsToUpdate.length} incidents...`);
			...
			logger.error("❌ Error recalculating durations:", error);
			toast({
				title: "Error",
	...
	const validateDatabase = async () => {
		try {
			...
			if (result.errors.length > 0) {
				logger.error("Database validation errors:", result.errors);
				toast({
	...
	return (
		<PageWrapper maxW="4xl">
			<div className="space-y-6 lg:space-y-8">
				<PageHeader
					title="Incident Data"
					description="Upload and manage incident records"
				/>
				{/* Header */}
					...
					<div className="bg-gray-50 dark:bg-gray-900/20 rounded-lg p-4">
						...
						<div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
							<div>
								...
								<strong>{dbStats.totalIncidents}</strong>
							</div>
							<div>
								• Unique No Cases: <strong>{dbStats.uniqueNoCases}</strong>
							</div>
							<div>
								• Unique Start Times:{" "}
								<strong>{dbStats.uniqueStartTimes}</strong>
							</div>
							{dbStats.duplicateGroups > 0 && (
								<div className="text-yellow-600 dark:text-yellow-400">
									<strong>{dbStats.duplicateGroups} duplicate groups</strong>{" "}
									detected. Consider cleaning duplicates.
								</div>
							)}
						</div>
					</div>
				...
				{filter.dateFrom ||
					...
					<div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
						...
						<div className="text-sm text-blue-700 dark:text-blue-300">
							Showing {filteredDataSummary.total} of {allDataSummary.total}{" "}
							incidents
							{filter.dateFrom && (
								<span className="ml-2">
									• Date range: {new Date(filter.dateFrom).toLocaleDateString()}{" "}
									-{" "}
									{filter.dateTo
										? new Date(filter.dateTo).toLocaleDateString()
										: "Now"}
								</span>
							)}
							{filter.status && (
								<span className="ml-2">• Status: {filter.status}</span>
							)}
							{filter.priority && (
								<span className="ml-2">• Priority: {filter.priority}</span>
							)}
							{filter.site && (
								<span className="ml-2">• Site: {filter.site}</span>
							)}
							{filter.search && (
								<span className="ml-2">• Search: "{filter.search}"</span>
							)}
						</div>
					</div>
				) : null}

				{showUpload && <IncidentUpload />}
					...
					<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
						<div className="bg-card text-card-foreground  rounded-lg p-6 max-w-md w-full mx-4">
							...
							<p className="text-gray-700 dark:text-gray-300 mb-6">
								Are you sure you want to delete all incident data? This will
								permanently remove all uploaded incidents from the database.
							</p>
				...
				{showCleanupConfirm && (
					<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
						<div className="bg-card text-card-foreground  rounded-lg p-6 max-w-md w-full mx-4">
							...
							<p className="text-gray-700 dark:text-gray-300 mb-6">
								This will remove duplicate incidents based on No Case and Start
								Time. Only the first occurrence of each duplicate will be kept.
								This action cannot be undone.
							</p>
					...
					<CardContent className="p-4">
						<div className="flex items-center gap-4">
							...
							<select
								value={filter.ncal || ""}
								onChange={(e) =>
									handleFilterChange("ncal", e.target.value || undefined)
								}
								className="px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm h-9"
							>
					...
					allIncidents.length > 0 && (
						<div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
							<div className="text-sm text-yellow-700 dark:text-yellow-300">
								No valid months found in data. Please check that the "Start"
														...
														{columns.map((col) => {
															...
															if (col.key === "durationMin") {
																...
																if (incident.startTime && incident.endTime) {
																	...
																	const isProblematic =
																		isProblematicDuration(calculatedDuration);
																	...
																	if (
																		...
																		displayValue = (
																			<div
																				className="text-xs font-mono font-medium text-red-500 dark:text-red-400"
																...
																const isProblematic =
																	isProblematicDuration(totalPauseMinutes);
																...
																if (
																	...
																	vendorDuration = calculateDuration(
																		incident.startEscalationVendor,
																		incident.endTime,
																	);
																}
																...
																const totalVendorDuration = Math.max(
																	vendorDuration - pauseDuration,
																	0,
																);

																// TOLAK durasi yang bermasalah seperti 07:14:19, 13:34:30, 05:14:28
																const isProblematic =
																	isProblematicDuration(totalVendorDuration);
															...
															return (
																<td
																	key={col.key}
																	className={`px-4 py-4 text-sm text-gray-900 dark:text-gray-100 align-top border-r border-gray-200 dark:border-gray-700 last:border-r-0 ${col.width || ""}`}
																>
																	<div
