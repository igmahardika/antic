...
const TSAnalytics: React.FC = () => {
		...
		try {
			...
			const validIncidents = incidents.filter((incident) => {
				if (!incident.id || !incident.noCase) {
					logger.warn("❌ TSAnalytics: Found invalid incident:", incident);
					return false;
				}
				return true;
			});
			...
			if (validIncidents.length !== incidents.length) {
				logger.warn(
					`❌ TSAnalytics: Filtered out ${incidents.length - validIncidents.length} invalid incidents`,
				);
			}
			...
			logger.error(
				"❌ TSAnalytics: Failed to load incidents from database:",
				error,
			);
			return [];
		}
	...
	useEffect(() => {
		...
		if (allIncidents && allIncidents.length > 0) {
			...
			logger.info("TS values found:", [
				...new Set(allIncidents.map((i) => i.ts)),
			]);
	...
	const getDurationMinutes = (incident: any): number => {
		if (Number.isFinite(incident?.durationMin) && incident.durationMin > 0)
	...
	const getWanedaDuration = (incident: any): number => {
		...
		if (
			Number.isFinite(incident?.totalDurationVendorMin) &&
		...
		dur -= overlapMinutes(
			vStart,
			vEnd,
			toDate(incident?.startPause1),
			toDate(incident?.endPause1),
		);
		dur -= overlapMinutes(
			vStart,
			vEnd,
			toDate(incident?.startPause2),
			toDate(incident?.endPause2),
		);
	...
	const getVendorDuration = (incident: any): number => {
		...
		if (
			vendor.includes("waneda") ||
			vendor.includes("lintas") ||
			vendor.includes("fiber")
		) {
		...
		if (
			Number.isFinite(incident?.durationVendorMin) &&
	...
	const calculatePowerDifference = (
		powerBefore: number,
		powerAfter: number,
	): number => {
		if (
	...
	const analyticsData = useMemo(() => {
		...
		const vendorIncidents = filteredIncidents.filter(
			(i) => categorizeTS(i.ts) === "vendor",
		);
		const internalIncidents = filteredIncidents.filter(
			(i) => categorizeTS(i.ts) === "internal",
		);
		// Vendor analytics
		const vendorStats = {
		...
		if (vendorIncidents.length > 0) {
			...
			const escalated = vendorIncidents.filter(
				(i) => i.startEscalationVendor,
			).length;
			vendorStats.escalationRate = (escalated / vendorIncidents.length) * 100;
			const powerValid = vendorIncidents.filter(
				(i) => i.powerBefore !== null && i.powerAfter !== null,
			);
			...
			vendorStats.powerCompliance =
				powerValid.length > 0 ? (powerCompliant / powerValid.length) * 100 : 0;
			// Group by vendor
			vendorIncidents.forEach((incident) => {
				...
				if (incident.powerBefore !== null && incident.powerAfter !== null) {
					const powerDiff = calculatePowerDifference(
		...
		if (internalIncidents.length > 0) {
			const validDurations = internalIncidents
				...
				.filter((d) => d > 0);
			internalStats.avgDuration =
				validDurations.length > 0
					? validDurations.reduce((a, b) => a + b, 0) / validDurations.length
					: 0;
			const escalated = internalIncidents.filter(
				(i) => i.startEscalationVendor,
			).length;
			internalStats.escalationRate =
				(escalated / internalIncidents.length) * 100;
			// Group by TS
	...
	const wanedaStats = useMemo(() => {
		...
		const items = Object.values(monthMap)
			.sort((a: any, b: any) => {
			...
			.map((data: any) => {
				const avgDuration =
					data.total > 0 ? data.totalDuration / data.total : 0;
				const avgExtra =
					data.nonTarget > 0 ? data.extraDuration / data.nonTarget : 0;
				...
				const avgPowerDiff =
					data.countPowerDiff > 0
						? data.totalPowerDiff / data.countPowerDiff
						: 0;
				return {
					month: data.month,
					total: data.total,
		...
		const avgExtraTotal =
			acc.nonTarget > 0 ? acc.extraDuration / acc.nonTarget : 0;
		const avgPowerDiffTotal =
			acc.countPowerDiff > 0 ? acc.totalPowerDiff / acc.countPowerDiff : 0;
	...
	const wanedaMonthlyViewData = fiscalMonths.map((month) => {
		...
		if (item) {
			...
			item.incidents.forEach((inc: any) => {
				if (
					typeof inc.powerBefore === "number" &&
					typeof inc.powerAfter === "number" &&
					!isNaN(inc.powerBefore) &&
					!isNaN(inc.powerAfter)
				) {
			...
			const avgPowerBetween =
				powerDiffs.length > 0
					? Number(
							(
								powerDiffs.reduce((a, b) => a + b, 0) / powerDiffs.length
							).toFixed(2),
						)
					: 0;
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
					...
					<SummaryCard
						...
						value={formatDurationHMS(
							analyticsData.vendor.avgDuration ||
								analyticsData.internal.avgDuration,
						)}
						...
						<CardContent className="p-6">
							<div className="space-y-4">
								...
								<div className="space-y-3">
									...
									{Object.entries(analyticsData.vendor.byVendor).map(
										([vendor, data]) => (
											<div
												key={vendor}
						...
						<CardContent className="p-6">
							<div className="space-y-4">
											...
											>
												<div className="flex items-center gap-2">
													<div
														...
														style={{
															backgroundColor:
																NCAL_COLORS[ncal as keyof typeof NCAL_COLORS],
														}}
													/>
													<div>
												...
												<div className="text-right">
													...
													<div className="text-xs text-muted-foreground">
														Target:{" "}
														{formatDurationHMS(
															NCAL_TARGETS[ncal as keyof typeof NCAL_TARGETS],
														)}
													</div>
												</div>
											</div>
								...
								<div className="space-y-3">
									...
									{Object.entries(analyticsData.internal.byTS)
										.slice(0, 5)
										.map(([ts, data]) => (
											<div
												key={ts}
										...
										data={(() => {
											...
											return Array.from(allMonths)
												...
												.map((month) => ({
													month,
													vendor:
														analyticsData.vendor.byMonth[month]?.total || 0,
													internal:
														analyticsData.internal.byMonth[month]?.total || 0,
												}));
										})()}
									>
										<CartesianGrid vertical={false} />
				...
				<Card>
					...
					<CardContent className="p-6">
						...
						<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
							...
							<div className="space-y-2">
								<h4 className="text-sm font-semibold text-muted-foreground">
									Case Count (Target vs Non-Target)
								</h4>
								<ChartContainer
									config={{
						...
						<div className="overflow-x-auto mt-6">
							...
							{(() => {
								const categories = [
									...
									{
										key: "avgExtra",
										label: "Avg Extra Time",
										color: "text-red-600",
									},
									{
										key: "avgPowerBetween",
										label: "Avg Power Between",
										color: "",
									},
								...
								const totalsMap: Record<string, any> = {
									...
									avgPowerBetween: avg(
										monthsWithData.map((m) => m.avgPowerBetween),
									),
									actualSlaPercent: avg(
										monthsWithData.map((m) => m.actualSlaPercent),
									),
									...
									<table className="min-w-full text-xs text-left">
										<thead className="bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-gray-300">
											<tr>
												...
												<th className="px-3 py-2 font-semibold text-right bg-blue-50 dark:bg-blue-900/20">
													TOTAL
												</th>
											</tr>
										</thead>
										<tbody>
											{categories.map((cat) => (
												...
												>
													...
													<td
														className={`px-3 py-2 text-right font-semibold bg-blue-50 dark:bg-blue-900/20 ${cat.color}`}
													>
														{(() => {
										...
										<tbody>
											{wanedaStats.items
												...
												?.incidents.map((incident: any, idx: number) => {
													...
													const targetStr =
														formatDurationHMS(VENDOR_SLA_MINUTES);
													const diffMin = duration - VENDOR_SLA_MINUTES;
													const diffStr =
														(diffMin < 0 ? "-" : "") +
														formatDurationHMS(Math.abs(diffMin));
													const performance =
														duration <= VENDOR_SLA_MINUTES
															? 100
															: (VENDOR_SLA_MINUTES / duration) * 100;
													...
													const powerDiff =
														...
														!isNaN(incident.powerAfter)
															...
															: "N/A";
													const isPowerOverTarget =
														typeof incident.powerAfter === "number" &&
														!isNaN(incident.powerAfter) &&
														incident.powerAfter > POWER_DELTA_OK_DBM;
													...
													return (
														...
														>
															...
															<td
																className={`px-2 py-2 text-right ${isOverTarget ? "text-red-600 dark:text-red-400 font-semibold" : ""}`}
															>
															...
															<td
																className={`px-2 py-2 text-right ${isPowerOverTarget ? "text-red-600 dark:text-red-400 font-semibold" : ""}`}
															>
																{powerAfter}
															</td>
															<td
															...
															<td className="px-2 py-2 whitespace-pre-wrap">
																{note}
															</td>
														</tr>
													);
